        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#E9EAEB]">
              <th className="text-left text-sm font-medium text-[#717680] px-6 py-3">Minimum Recurring Investment (USDC)</th>
              <th className="text-left text-sm font-medium text-[#717680] px-6 py-3">Maximum Recurring Investment (USDC)</th>
              <th className="text-left text-sm font-medium text-[#717680] px-6 py-3">Applicable to</th>
            </tr>
          </thead>
          <tbody>
            {dcaLimits.map((limit, index) => (
              <React.Fragment key={index}>
                <tr className="border-b border-[#E9EAEB]">
                  <td className="px-6 py-4">
                    <input
                      type="text"
                      value={limit.minRecurring}
                      onChange={(e) => {
                        const newLimits = [...dcaLimits];
                        newLimits[index].minRecurring = e.target.value;
                        setDcaLimits(newLimits);
                      }}
                  ...
                  <td className="px-6 py-4 border-r border-[#E9EAEB]">
                    <input
                      type="text"
                      value={limit.maxRecurring}
                      onChange={(e) => {
                        const newLimits = [...dcaLimits];
                        newLimits[index].maxRecurring = e.target.value;
                        setDcaLimits(newLimits);
                      }}
                ...
                {limit.applicableTo === "Specific Customers" && (
                  <tr className={index === dcaLimits.length - 1 ? "" : "border-b border-[#E9EAEB]"}>
...
export default DCALimitsSection;
